...
local gui_job = require("modules/ctron_plugin/script/ui_job_screen")

-- injected by control.lua after all modules are loaded to avoid circular dependency
---@type table
local clusterio_handler = nil

local entity_proc = {}
---@param handler table
entity_proc.set_clusterio_handler = function(handler)
    clusterio_handler = handler
end
...
entity_proc.new_station_built = function(entity, surface_index)
    ...
    clusterio_handler.station_built(entity)
end
...
entity_proc.on_ghost_entity_died = function(event)
...
entity_proc.on_marked_for_deconstruction = function(event)
    local entity = event.entity
    local surface_index = entity.surface.index
...
-- for entity upgrade
entity_proc.on_marked_for_upgrade = function(event)
    local entity = event.entity
    local surface_index = entity.surface.index
...
-- for entity repair
entity_proc.on_entity_damaged = function(event)
    local entity = event.entity
    local surface_index = entity.surface.index
...
entity_proc.on_entity_cloned = function(event)
...
entity_proc.script_raised_teleported = function(event)
    local entity = event.entity
...
entity_proc.on_object_destroyed = function(event)
    ...
    elseif storage.station_names[removed_entity.name] then
        ...
        clusterio_handler.station_removed()
...
-- left click
entity_proc.on_player_selected_area = function(event)
...
-- right click
entity_proc.on_player_reverse_selected_area = function(event)
    if event.item ~= "ctron-selection-tool" then return end
    for _, entity in pairs(event.entities) do
        if entity and entity.valid then
...
-- shift left click
entity_proc.on_player_shift_selected_area = function (event)
    if event.item ~= "ctron-selection-tool" then return end
    local player = game.players[event.player_index]
    for _, entity in pairs(event.entities) do
...
-- shift right click
entity_proc.on_player_alt_reverse_selected_area = function(event)
    if event.item ~= "ctron-selection-tool" then return end
    for _, entity in pairs(event.entities) do
        if entity and entity.valid then
